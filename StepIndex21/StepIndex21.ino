    ...
    case mainmenu:                    // main menu
      switch(cur_key) 
      {
        ...
        case BUTTON_DOWN:
          ...
          break;
        case BUTTON_LEFT:           // left and right keys do nothing in main menu
          break;
        case BUTTON_RIGHT:
          break;
        case BUTTON_SELECT:         // user has picked a menu item
...
//  if (tmp_key == BUTTON_SELECT)       
//    return;
  while (1 == 1)                           // forever (until a Select Key)
  {        
    move_motor(1,cur_dir,0); 
    uint8_t buttons = lcd.readButtons();    // move motor 1 step
    if (buttons)  // if a keypress is present       {
    { 
      cur_key = lcd.readButtons();           // then get it
      switch(cur_key)                       // and honor it
      {
      case BUTTON_UP:                        // bump up the speed
        if (motorspeeddelay > SpeedDelayIncrement)
          motorspeeddelay -= SpeedDelayIncrement;
        break;
      case BUTTON_DOWN:                      // bump down the speed
        motorspeeddelay += SpeedDelayIncrement;
        break;
      case BUTTON_LEFT:                      // set direction
        cur_dir = CCW;
        displayscreen(runmode); 
        break;
      case BUTTON_RIGHT:                     // set other direction
        cur_dir = CW;
        displayscreen(runmode); 
        break;
      case BUTTON_SELECT:                   // user wants to stop
...
void dotempmode(int test_key)
{
  while (1)
  {
    if (test_key == BUTTON_SELECT) return;     // all we do here is wait for a select key
    test_key = lcd.readButtons();
  } 
  return;                                   // to exit
}
...
void dojogmode(int tmp_key)
{
  ...
  for (;breakflag==0;)
  {
    switch(tmp_key) 
    {
      case BUTTON_UP:                          // bump the number of steps
        numjogsteps += JogStepsIncrement;
        break;
      case BUTTON_DOWN:                        // reduce the number of steps
        if (numjogsteps > JogStepsIncrement)
          numjogsteps -= JogStepsIncrement;
          break;
      case BUTTON_LEFT:                        // step the motor CCW
          move_motor(numjogsteps,CCW,0);  
          break;
      case BUTTON_RIGHT:                       // step the motor CW
          move_motor(numjogsteps,CW,0);  
          break;
      case BUTTON_SELECT:                      // user want to quit
          breakflag = 1;
          break;
    }
    if (breakflag == 0)
